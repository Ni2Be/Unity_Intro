...
public class Planet_Generator : MonoBehaviour
{
    ...
    public void apply_height_map()
    {
        ...
        if (height_map_ == null)
        {
            Debug.Log("Heightmap was null in apply_height_map()");
    ...
    public void apply_textures()
    {
        ...
        if (is_star_)
            water_material_ = generate_lava(lava_type_);
    ...
    public Material generate_lava(float type)
    {
        Material lava       = new Material(lava_);
        
        Texture2D diffuse   = lava_.GetTexture("_MainTex") as Texture2D;
        Texture2D illumni   = lava_.GetTexture("_Illum") as Texture2D;
        Texture2D normal    = lava_.GetTexture("_BumpMap") as Texture2D;
        
        Texture2D new_diffuse   = new Texture2D(diffuse.width, diffuse.height);
        ...
        if (type < 0.2f)
        {
            for (int w = 0; w < diffuse.width; w++)
            {
                for (int h = 0; h < diffuse.height; h++)
                {
                    new_pixel = diffuse.GetPixel(w, h);
                    new_pixel = new Color(new_pixel.b, new_pixel.r, new_pixel.g);
                    new_diffuse.SetPixel(w, h, new_pixel);
                }
            }
        }
        //white blue
        else if (type < 0.4f)
        {
            for (int w = 0; w < diffuse.width; w++)
            {
                ...
                {
                    new_pixel = diffuse.GetPixel(w, h);
                    new_pixel = new Color(1 - new_pixel.r, 1 - new_pixel.g, 1 - new_pixel.b);
                    new_pixel.r -= 0.1f;
                    new_pixel.g -= 0.1f;
                    new_pixel.b -= 0.1f;
                    new_diffuse.SetPixel(w, h, new_pixel);
        ...
        else if (type < 0.6f)
        {
            for (int w = 0; w < diffuse.width; w++)
            {
                ...
                {
                    new_pixel = diffuse.GetPixel(w, h);
                    new_pixel = new Color(new_pixel.b, new_pixel.g, new_pixel.r);
                    new_diffuse.SetPixel(w, h, new_pixel);
                }
            }
        }
        ...
        new_diffuse.Apply();
        lava    .SetTexture("_MainTex", new_diffuse);
        lava    .SetTexture("_Illum", illumni);
        lava    .SetTexture("_BumpMap", normal);
...
#if UNITY_EDITOR
    [MenuItem("GameObject/Planet Generator/Celestrial Body - empty", false, 50)]
    public static Planet_Generator create_planet_generator()
    {
        GameObject go = new GameObject("Celestrial Body");
        Planet_Generator planet_generator = go.AddComponent<Planet_Generator>();
        
        return planet_generator;
    }

    [MenuItem("GameObject/Planet Generator/Celestrial Body - terra", false, 50)]
    public static Planet_Generator create_planet_generator_terra()
    {
        GameObject go = new GameObject("Celestrial Body");
        Planet_Generator planet_generator = go.AddComponent<Planet_Generator>();

        planet_generator.generate_terra();
        return planet_generator;
    }

    [MenuItem("GameObject/Planet Generator/Celestrial Body - star", false, 50)]
    public static Planet_Generator create_planet_generator_star()
    {
        GameObject go = new GameObject("Celestrial Body");
        Planet_Generator planet_generator = go.AddComponent<Planet_Generator>();

        planet_generator.generate_star();
        return planet_generator;
    }

    [MenuItem("GameObject/Planet Generator/Celestrial Body - hot", false, 50)]
    public static Planet_Generator create_planet_generator_hot()
    {
        GameObject go = new GameObject("Celestrial Body");
        Planet_Generator planet_generator = go.AddComponent<Planet_Generator>();

        planet_generator.generate_hot();
        return planet_generator;
    }
#endif
    #endregion
    ...
    public void generate_star()
    {
        //generate mesh and texture
        is_star_        = true;

        lava_type_      = Random.Range(0.0f, 1.0f);
